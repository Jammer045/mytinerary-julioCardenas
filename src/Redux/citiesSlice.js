...
export const fetchCityById = createAsyncThunk(
    ...
    async (id, { rejectWithValue }) => {
      try {
        ...
        const data = await response.json();
        console.log('City data received:', data); // Para debugging
        return data.response;
      } catch (err) {
  ...
  reducers: {
    ...
    nextSlide: (state) => {
        state.currentIndex = (state.currentIndex + 1) % Math.ceil(state.destinations.length / 4);
      },
      prevSlide: (state) => {
        state.currentIndex = (state.currentIndex - 1 + Math.ceil(state.destinations.length / 4)) % Math.ceil(state.destinations.length / 4);
      },
    },
...
export const { searchCities, nextSlide, prevSlide, clearCurrentCity } = citiesSlice.actions;
